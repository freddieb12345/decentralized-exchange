...
contract Exchange is ERC20 {
    ...
    constructor(address _CryptoDevtoken) ERC20("CryptoDev LP Token", "CDLP") {
        require(_CryptoDevtoken != address(0), "Token address passed is a null adress");
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        if(cryptoDevTokenReserve == 0) {
            ...
            // "Liquidity" provided is equal to "ethBalance" because this is the first time user is adding "Eth" to the contract, so whatever
            // "Eth" contract has is equal to the one supplied by the user in the current "addLiquidity" call
            // "Liquidity" tokens that need to be minted to the user on "addLiquidity" call should always be proportional to the Eth specified
            // to the user
            ...
            // EthReserve shold be the current ethBalance subtracted by the value of ether sent by the user
    ...
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // The amount of Eth that would be sent back the the user is based on a ratio
        // Ratio is -> (Eth sent back to the user) / (Current Eth reserve) = (amount of LP tokens taht user wants to withdraw) / (total supply of LP tokens)
        ...
        // Transfer "cryptoDevTokenAmount" of Crypto Dev Tokens from the user's waller to the contract
